export function RestaurantLinks({ name, area, googlePlaceId }: { name: string; area?: string | null; googlePlaceId?: string | null }) {
  ...
  return <nav aria-label={`Enllaços externs de ${name}`} className="mt-4 grid grid-cols-2 gap-2 sm:flex sm:flex-wrap">
    {links.map(link => <a key={link.label} href={link.href} target="_blank" rel="noopener noreferrer" aria-label={`${link.label}: ${name} (s’obre en una pestanya nova)`} className="flex items-center justify-center gap-1 rounded-lg border border-slate-300 bg-white px-3 py-2 text-xs font-semibold text-slate-700 hover:border-orange-400 hover:text-orange-800 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-orange-600">{link.label} <span aria-hidden="true">↗</span></a>)}
  </nav>;
}
