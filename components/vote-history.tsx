...
export function VoteHistory({ votes, personal = false }: { votes: HistoryVote[]; personal?: boolean }) {
  return (
    <section className="mt-8 rounded-3xl border border-orange-100 bg-white p-4 sm:p-8">
      ...
      {votes.length ? <ul className="mt-6 divide-y divide-slate-100">
        {votes.map(vote => <li key={vote.id} className="flex flex-col items-start justify-between gap-3 py-5 sm:flex-row sm:items-center">
          <div className="min-w-0">
            ...
            <RestaurantLinks name={vote.restaurantName} area={vote.restaurantArea} />
