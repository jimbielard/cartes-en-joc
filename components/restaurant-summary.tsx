...
export function RestaurantSummary({ summary, restaurant }: { summary?: Summary; restaurant?: PlaceRestaurant }) {
  ...
  return <div className="category-summary mt-4 border-t border-slate-200 pt-4">
    <p className="flex flex-wrap items-baseline gap-x-3 gap-y-1 text-sm text-slate-700"><strong className="text-lg text-orange-800">{summary?.average == null ? "Sense puntuació" : `${summary.average.toFixed(1)} / 10`}</strong><span className="block">{summary?.totalVotes ?? 0} valoracions totals</span></p>
